import {modifyDoorState, modifyLightState, modifyWindowState} from "../../modules/HouseOverview/HouseService";
...
export const houseObjectStateChangeHandler = async (event, itemType) => {
    ...
    switch (itemType) {
        ...
        case 'exteriorDoorOpen' :
            await modifyDoorState(itemID, true, event.target.checked);
            break;

        case 'exteriorDoorLock' :
            await modifyDoorState(itemID, false, event.target.checked)
            break;
